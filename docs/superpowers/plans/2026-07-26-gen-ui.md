...
- Produces:
  - `func uiDestPath(registryPath string) string` — project-relative destination
...
Add to `cmd/goappctl/internal/scaffold/ui.go` (and add `"strings"` to its imports):
...
**Interfaces:**
- Consumes: `resolveItems`, `uiDestPath`, `rewriteRegistryImports`.
- Produces:
...
func TestUI_DryRunWritesNothing(t *testing.T) {
	...
	if !strings.Contains(out.String(), "ui/button/Button.vue") {
...
func UI(names []string, opts UIOptions) error {
	...
	// Refuse before the first write rather than partway through.
	if !opts.Force && !opts.DryRun {
		for _, item := range items {
			for _, f := range item.Files {
				full := filepath.Join(opts.ModuleRoot, uiDestPath(f.Path))
	...
	for _, item := range items {
		for _, f := range item.Files {
			dest := uiDestPath(f.Path)
			fmt.Fprintf(out, "  %s -> %s\n", f.Path, dest)
			if opts.DryRun {
				continue
			}
			full := filepath.Join(opts.ModuleRoot, dest)
			if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
				return fmt.Errorf("gen ui: mkdir for %s: %w", dest, err)
			}
			if err := os.WriteFile(full, []byte(rewriteRegistryImports(f.Content)), 0o644); err != nil {
				return fmt.Errorf("gen ui: write %s: %w", dest, err)
			}
		}
	}
	return nil
