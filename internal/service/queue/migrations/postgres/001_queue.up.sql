...
--
-- Every timestamp is BIGINT UnixNano, never TIMESTAMPTZ: the column names and
-- shapes are identical across the three dialects, which is what lets the Go code
-- hold one set of queries. INTEGER is 32-bit here and would overflow 2.1 seconds
-- after 1970.
--
-- No TEXT column carries a constant DEFAULT — the Go code binds every column on
-- INSERT. That is not a PostgreSQL constraint; it exists so the MySQL twin does
-- not have to promote payload-sized columns to VARCHAR(n) and gain a length
-- ceiling this dialect does not have.
...
--
-- No CHECK constraint and no ENUM on status: the vocabulary lives in the Go
-- constants, and a CHECK here would be a second place to edit, enforced on one
-- dialect's terms only.
--
...
--
-- schedule_id carries no foreign key. PostgreSQL would enforce one, SQLite would
-- not (this template does not set PRAGMA foreign_keys=ON) — the dialect
-- divergence migrations/README.md warns about. The index is what the queries
-- need anyway.
--
...
-- not "unique among non-terminal rows": PostgreSQL could express that with a
-- partial index, MySQL could not, and a constraint that exists on two dialects
-- out of three is worse than one that behaves the same everywhere. Callers
-- wanting periodic deduplication put a time bucket in the key, which is what the
-- cron producer does.
...
--
-- enabled and present are SMALLINT 0/1 rather than BOOLEAN: the column shapes
-- stay identical across the three dialects, so one set of Go queries scans them
-- into an int and compares != 0.
--
