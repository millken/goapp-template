...
func TestUI_DryRunWritesNothing(t *testing.T) {
	...
	for _, dir := range []string{"frontend/src/components", "frontend/src/lib"} {
...
// The overwrite check must cover the whole plan before the first write. utils is
// a dependency, so it is written first; pre-creating only button's destination —
// the last entry — means a weaker check-then-write-per-item loop would already
// have written utils.ts by the time it noticed.
func TestUI_ExistingLaterFileStopsEarlierWrites(t *testing.T) {
	root := t.TempDir()
	buttonDest := filepath.Join(root, "frontend/src/components/ui/button/Button.vue")
	if err := os.MkdirAll(filepath.Dir(buttonDest), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(buttonDest, []byte("mine"), 0o644); err != nil {
		t.Fatal(err)
	}

	var out bytes.Buffer
	err := UI([]string{"button"}, UIOptions{ModuleRoot: root, BaseURL: twoItemStub(t), Out: &out})
	if err == nil || !strings.Contains(err.Error(), "--force") {
		t.Fatalf("want an overwrite refusal, got %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, "frontend/src/lib/utils.ts")); err == nil {
		t.Error("an earlier plan entry was written before the later conflict was found")
	}
	got, _ := os.ReadFile(buttonDest)
	if string(got) != "mine" {
		t.Errorf("the existing file was overwritten: %q", got)
	}
}

// Two components claiming one destination is a registry problem; letting the
// second silently win would hide it.
func TestUI_RefusesCollidingDestinations(t *testing.T) {
	base := registryStub(t, map[string]registryItem{
		"clash": {Name: "clash", Files: []registryFile{
			{Path: "ui/x/A.vue", Content: "first"},
			{Path: "ui/x/A.vue", Content: "second"},
		}},
	})
	root := t.TempDir()
	var out bytes.Buffer
	err := UI([]string{"clash"}, UIOptions{ModuleRoot: root, BaseURL: base, Out: &out})
	if err == nil || !strings.Contains(err.Error(), "both write") {
		t.Fatalf("want a collision error, got %v", err)
	}
	if _, err := os.Stat(filepath.Join(root, "frontend/src/components/ui/x/A.vue")); err == nil {
		t.Error("a colliding destination was written anyway")
	}
}
